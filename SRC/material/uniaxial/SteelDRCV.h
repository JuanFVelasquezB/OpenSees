...
class SteelDRCV : public UniaxialMaterial
{
  public:
	...
	const char *getClassType(void) const {return "SteelDRCV";};
  ...
  private:
	...
	double kf;      // Viscous damper constant applied to strain rate
	double np_visc; // Viscous damper exponent applied to strain rate
...
/* =======================================================================================================*/
    ...
    /* =======================================================================================================*/
	// PINCHING VARIABLES (Written by Juan Fernando Velásquez Bedoya jfvelasq@unal.edu.co)

	int fQ;                 // Estado de la máquina histérica de pinching

	std::string Qa;         // Cuadrante previo convergido
	std::string Ca;         // Carga previo	

	double bf1;             // Función beta principal
	double bf2;             // Función beta secundaria
	double bfc;             // Derivada de beta principal

	double brb;             // Beta en punto de reversa
	double erb;             // Strain en reversa
	double frb;             // Stress en reversa

	double bra;             // Beta en recarga
	double era;             // Strain en recarga
	double fra;             // Stress en recarga

	double e0;              // Punto de cruce por cero
	double e1;              // Límite de activación pinching
	double ed;              // Reducción dominio efectivo

	double lambda;          // Parámetro de pinching
	double np;              // Exponente función beta

	double bf[2];           // Factores beta
	double bfd[2];          // Derivadas beta

	int p;                  // Índice dirección	
